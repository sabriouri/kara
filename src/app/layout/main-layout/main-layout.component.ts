...
export class MainLayoutComponent {
  ...
  readonly poles: Pole[] = [
    ...
    },
    {
      ...
      color: '#52AE4F',
      dashboardRoute: '/dashboard',
      nav: [{ label: 'Virements', icon: 'landmark', route: '/virements' }, ...this.commonNav],
    },
    {
      id: 'projet',
      label: 'Pôle Projet',
  ...
  activePole = computed(
    () => this.poles.find((p) => p.id === this.activePoleId()) ?? this.poles[0],
  );
  ...
  openFlyout(event: MouseEvent, item: NavItem): void {
    ...
    if (this._flyoutClose) {
      clearTimeout(this._flyoutClose);
